...
G4VPhysicalVolume *MyDetectorConstruction::Construct()
{
	...
	*/
    ...
    G4Material *worldMat = Air;
   	...
   	G4Box *solidWorld = new G4Box("solidWorld", 0.4*m, 0.4*m, 0.05*m);
    	G4LogicalVolume *logicWorld = new G4LogicalVolume(solidWorld, worldMat, "logicWorld");
    	G4VPhysicalVolume *physWorld = new G4PVPlacement(0, G4ThreeVector(0., 0., 0.), logicWorld, "physWorld", 0, false, 0, true);

	//teflon shell in PET cell
	G4Tubs *solidTeflonShell = new G4Tubs("teflonShell", 0.3*m, 0.36*m, 0.03*m, 0.*deg, 360.*deg);
	...
	G4Tubs *solidLiquidXenon = new G4Tubs("liquidXenon", 0.305*m, 0.355*m, 0.025*m, 0.*deg, 360.*deg);
	...
	//liquid xenon in PET cell for detection
	//G4Tubs *solidLiquidXenonDetector = new G4Tubs("liquidXenonDetector", 0.305*m, 0.306*m, 0.025*m, 0.*deg, 360.*deg);
	//G4LogicalVolume *logicLiquidXenon = new G4LogicalVolume(solidLiquidXenon, G4Material::GetMaterial("LXe"), "logicLiquidXenon");
	//G4LogicalVolume *logicLiquidXenonDetector = new G4LogicalVolume(solidLiquidXenonDetector, LXe, "logicLiquidXenonDetector");
	//G4VPhysicalVolume *physLiquidXenonDetector = new G4PVPlacement(0, G4ThreeVector(0., 0., 0.0025*m), logicLiquidXenonDetector, "physLiquidXenonDetector", logicLiquidXenon, false, 0, true);

	fScoringVolume = logicLiquidXenon;
